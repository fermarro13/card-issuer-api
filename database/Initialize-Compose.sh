...
for name in CONTROL_DB_PASSWORD AUTH_DB_PASSWORD SHARD_DB_PASSWORD EXECUTOR_DB_PASSWORD; do
    case "$name" in
        ...
        EXECUTOR_DB_PASSWORD) password=${EXECUTOR_DB_PASSWORD-} ;;
    esac
...
PGSSLMODE=disable
export PGSSLMODE
for connection in "${control_database}|ci_app_control|${CONTROL_DB_PASSWORD}" "${control_database}|ci_app_auth|${AUTH_DB_PASSWORD}" "${shard_database}|ci_app_shard|${SHARD_DB_PASSWORD}" "${control_database}|ci_app_executor|${EXECUTOR_DB_PASSWORD}" "${shard_database}|ci_app_executor|${EXECUTOR_DB_PASSWORD}"; do
