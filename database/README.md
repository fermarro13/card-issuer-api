...
Creates [the project design](../docs/database-design.md): a central control database (five tables) and one bank shard (fourteen tables). PostgreSQL **17** is the tested baseline. No PostgreSQL extensions are required.
...
Card transition rules, already-at-target behavior, local issuance/replacement orchestration, and automatic version advancement remain application responsibilities. A status domain does not validate transitions. V1 issuance/replacement synchronously creates an issued card, succeeded operation, opaque non-secret credential reference, history, audit evidence, and idempotency result; no provider confirmation or outbox work is involved. Accepted-batch submission must atomically create all membership, operations, and idempotency records; the application validates nonempty/exact counts and prevents later item insertion. Header/request identities and existing item references are immutable, but the schema alone is not a batch admission or execution API.
