...
$settings=@{
    ...
    POSTGRES_PASSWORD='Dev-Postgres-Admin!2026'; CONTROL_DB_PASSWORD='Dev-Control-Reader!2026'; SHARD_DB_PASSWORD='Dev-Shard-Runtime!2026'
}
...
try {
    ...
    Assert ((Query 'postgres' "SELECT count(*) FROM pg_roles WHERE rolname IN ('ci_app_control','ci_app_shard') AND NOT (rolsuper OR rolcreatedb OR rolcreaterole OR rolbypassrls OR rolreplication);") -eq '2') 'Runtime privilege attributes are incorrect.'
